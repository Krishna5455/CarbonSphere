...
# Emission factors (GLEC / IPCC standard estimates)
TRANSPORT_EMISSION_FACTOR_PER_TONNE_KM = 0.000096 # tCO2e per tonne-km (Heavy-duty diesel freight)
GRID_EMISSION_FACTOR_PER_KWH = 0.00082 # tCO2e per kWh (Indian regional grid average)
...
def calculate_carbon_metrics(
    ...
    if pathway == PathwayType.BIOCHAR:
        # Avoided open crop burning or uncontrolled pile rot
        # Crop burning emission factor: ~1.15 tCO2e avoided per tonne dry biomass
        gross_avoided = dry_tonnes * 1.15
        
        # Permanent biogenic carbon storage in biochar
        ...
        sequestration = biochar_tonnes * 0.78 * (44.0 / 12.0) * 0.80
        
    elif pathway == PathwayType.BIOGAS:
        # Avoided landfill anaerobic methane formation
        # IPCC FOD model: ~0.85 tCO2e avoided per tonne wet organic waste diverted from landfill
        gross_avoided = quantity * 0.85
        
        # Biomethane displacing fossil natural gas
        ...
        sequestration = (cbg_yield_kg * 2.2) / 1000.0 # fossil displacement counted as permanent reduction
        
    elif pathway == PathwayType.CARBON_MATERIALS:
        # Avoided landfill / incineration
        gross_avoided = dry_tonnes * 0.90
        
        # Embodied carbon displacement: substituting cement or synthetic resins
        # Displacement credit ~1.3 tCO2e per tonne composite produced
        composite_tonnes = dry_tonnes * float(facility.get("byproduct_yield_factor", 0.65))
        sequestration = composite_tonnes * 1.30
        
    net_carbon_impact = gross_avoided + sequestration - process_emissions - transport_emissions
    
    return {
        "gross_carbon_avoided_tco2e": round(gross_avoided, 3),
        "permanent_sequestration_tco2e": round(sequestration, 3),
