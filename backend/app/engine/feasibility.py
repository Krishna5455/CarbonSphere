...
def evaluate_feasibility(waste: WasteStreamInput, facility: dict) -> Tuple[bool, List[str], float]:
    ...
    # 3. Pathway Specific Technical Chemistry Rules
    ...
    if pathway == PathwayType.BIOCHAR:
        ...
        if "agricultural" in waste.waste_type.lower() or "bagasse" in waste.waste_type.lower():
            # Raw crop stalks lack tensile fiber consistency without chemical retting
            reasons.append(f"Feedstock '{waste.waste_type}' lacks structural fiber consistency required for carbon-negative composite panels.")
